...
def datetime_formatter(num_days, num_hours): # returns a timestamp of num_days before the current time
    ...
    if int(split_now_string_day[0]) == 0:
        ...
        datetime_result = datetime(int(split_now_string[0]), int(split_now_string[1]), int(split_now_string_day[0]) - 1, 0, 0, 0)
...
def API_get_states(API_States_Dict, current_user):
    ...
    Access_Info_String = current_user + "---" + str(AccessTimestamp) + "---" +"States"+"---"+ str(converted_dict["origin_country"])
...
def API_get_arrivals_by_airport(API_States_Dict, current_user, airport_code, socket_conn, Login_Dict, Clearance_Dict):
    api = OpenSkyApi()
    #now = datetime.now()
    one_day = datetime_formatter(1,0)
    unix_timestamp_now = (datetime.now() - datetime(1970,1,1, 0, 0)).total_seconds()
    unix_timestamp_previous_day = (one_day - datetime(1970,1,1)).total_seconds()

    AccessTimestamp = datetime.now()
    converted_dict = {}
    
    #arrivals = api.get_arrivals_by_airport(airport_code, int(unix_timestamp_previous_day), int(unix_timestamp_now))
    arrivals = api.get_arrivals_by_airport("EDDF", int(unix_timestamp_previous_day), int(unix_timestamp_now))
    print(type(arrivals))
    print(len(arrivals))
    #print(arrivals[1])
    
    for x in vars(arrivals[1]):
        converted_dict[x] = str(vars(arrivals[1])[x])
    
    #Access_Info_String = current_user + "---" + str(AccessTimestamp) + "---" +"Arrival---AirportCode_"+str(airport_code)
    Access_Info_String = current_user + "---" + str(AccessTimestamp) + "---" +"Arrival"+"---"+ "EDDF"
    print(Access_Info_String)
    API_States_Dict[Access_Info_String] = [converted_dict, 3]
    
    send_string = "Arrival at airport EDDF with access string: " + Access_Info_String
    socket_conn.sendall(send_string.encode('ascii'))
    data = socket_conn.recv(2048)
    
    socket_conn.sendall("API Request added to database")
    #data = socket_conn.recv(2048)
