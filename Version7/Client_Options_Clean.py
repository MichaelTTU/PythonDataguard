...
def add_user(socket_conn):
    ...
    if str(decoded_data) == "1001":
        ...
        login_pair[0] = empty_catcher("Enter a username for the new user: ")
        login_pair[1] = empty_catcher("Enter a password for the new user: ")          
...
def API_get_states(socket_conn):
    print(" ")
    data = socket_conn.recv(2048)
    decoded_data = data.decode('ascii')
    country_choice = empty_catcher(decoded_data)
    socket_conn.sendall(country_choice.encode('ascii'))
    data = socket_conn.recv(2048)
    decoded_data = data.decode('ascii')
    print(str(decoded_data))
...
def empty_catcher(input_string):
    item = input(input_string)
    if item == "":
        blank = True
        while(blank):
            item = input("Entry is blank. Please provide an input: ")
            if item != "":
                blank = False
    return item
...
def login(socket_conn):
    ...
    attempted_login[0] = empty_catcher("Enter your username: ")
    attempted_login[1] = empty_catcher("Enter your password: ")
    ...
    if str(decoded_data) == "626": # successful login
        print("Login as user " + attempted_login[0] + " Successful")
        options(socket_conn)
        pass
    else:
...
def request_api_call(socket_conn):
    ...
    data = socket_conn.recv(2048)
    decoded_data = data.decode('ascii')
    airport_choice = empty_catcher("Please provide an airport code to check recent flights from: ")
    ...
    if str(decoded_data) == "Arrivals API call failed. Invalid airport code or no flights in time window.":
        ...
        callsign_choice = empty_catcher("Please provide a specific aircraft callsign to search for (Most recent arrival is callsign "+ str(decoded_data)+"): ")
...
def options(socket_conn):
    ...
    print("1) Add a user")
    print("2) Logout")
    print("3) Make arrivals API call")
    print("4) View previous API calls")
    print("5) Make states API call")
...
def verify_input_choice(socket_conn, input_type):
    ...
    input_choice = empty_catcher(input_type + " (type number):")
